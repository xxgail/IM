...
const (
	serversHashKey       = "im:hash:servers" // 全部的服务器
...
func SetServerInfo(server *models.Server, currentTime uint64) (err error) {
	...
	number, err := redisClient.Do(common.Ctx, "hSet", key, server.String(), value).Int()
	if err != nil {
	...
	redisClient.Do(common.Ctx, "Expire", key, serversHashCacheTime)
...
func DelServerInfo(server *models.Server) (err error) {
	key := getServersHashKey()
	redisClient := redislib.GetClient()
	number, err := redisClient.Do(common.Ctx, "hDel", key, server.String()).Int()
	if err != nil {
...
func GetServerAll(currentTime uint64) (servers []*models.Server, err error) {
	...
	val, err := redisClient.Do(common.Ctx, "hGetAll", key).Result()

	valByte, _ := json.Marshal(val)
	fmt.Println("GetServerAll", key, string(valByte))

	serverMap, err := redisClient.HGetAll(common.Ctx, key).Result()
	if err != nil {
